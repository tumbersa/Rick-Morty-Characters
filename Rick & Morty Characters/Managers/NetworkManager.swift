...
final class NetworkManager {
    ...
    func fetchFilteredCharacters(page: Int, status: CharacterModel.Status?, gender: CharacterModel.Gender?) async throws -> [CharacterModel] {
        var endPoint = baseUrl + "/character/" + "?page=\(page)"
        if let status {
            endPoint += "&status=" + status.rawValue
        }
        if let gender {
            endPoint += "&gender=" + gender.rawValue
        }
